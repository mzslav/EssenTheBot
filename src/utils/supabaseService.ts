...
export async function saveMeal(
  ...
  return { ...data, emoji: getRandomEmoji(), type: inputMode };
}
...
export async function addMealToToday(
  ...
  return { ...data, emoji: emoji || getRandomEmoji(), type: 'text' as const };
}
...
export async function getRecentMeals(telegramUserId: number, limit = 5): Promise<MealRecord[]> {
  ...
  return (data || []).map((item: any) => ({
    ...
    created_at: item.created_at, _date: item.daily_logs?.date, emoji: getRandomEmoji(),
  }));
}
